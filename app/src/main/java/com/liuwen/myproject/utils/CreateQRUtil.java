...
public class CreateQRUtil {
    ...
    public static Bitmap createQRImage(Context context, String data, Bitmap logoBm) {
        try {
            ...
            // 设置空白边距的宽度
            hints.put(EncodeHintType.MARGIN, 0); //default is 4 3
            // 图像数据转换，使用了矩阵转换
            ...
            for (int y = 0; y < heightPix; y++) {
                for (int x = 0; x < widthPix; x++) {
                    if (bitMatrix.get(x, y)) {
                        pixels[y * widthPix + x] = context.getResources().getColor(R.color.colorQRCode);
                    } else {
                        pixels[y * widthPix + x] = context.getResources().getColor(R.color.colorWhite);
                    }
                }
            }
